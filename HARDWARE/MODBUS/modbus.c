...
float Hex2Float(unsigned long long number)
{
	...
	int exponent = ((number >> 23) & 0xff) - 127;//"e位
	float mantissa = 1 + ((float)(number & 0x7fffff) / 0x7fffff);//x位
...
void Host_Func3()
{
	...
	//[6]  Lbit数据
//	int i;
	float result=0;//转换为浮点数的值
	unsigned long long temp=0;//十进制IEEE754浮点数数值 0xBE40E612 =  -0.188378
	temp=(int)modbus.rcbuf[6]+(int)modbus.rcbuf[5]*256+(int)modbus.rcbuf[4]*256*256+((int)modbus.rcbuf[3])*256*256*256;
	result = Hex2Float(temp);
	printf("{\"result_pressure\":\"%f\"}", result);
